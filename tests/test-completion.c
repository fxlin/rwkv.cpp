// cf generate_completions.py 
// implement in C (by FL oct 2024
...
void test_on_prompt(const char * prompt, const size_t prompt_length) {
    ...
    for (int i = 0; i < 4; i++) {           // xzl: eval prompts in batch (sequence)
    ...
    // cf: test_serial_mode() 
    int tokens_per_generation = 200; 
    for (int i = 0; i < tokens_per_generation; i++) {
        printf("\n--- Generation {GENERATION} ---\n");

        rwkv_eval(ctx, prompt[i], state /* xzl: state in*/, 
            state /*xzl: state out*/,     logits);
        // sample logits... 
